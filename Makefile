...
object:
		...
		avr-ranlib libtwi.a
		avr-gcc $(cflags) -mmcu=$(avrType) -Wa,-ahlmns=uart.lst -o uart.o -c uart.c
		avr-ar rcsv libuart.a uart.o
		avr-ranlib libuart.a
		avr-gcc $(cflags) -mmcu=$(avrType) -Wa,-ahlmns=$(src).lst -o $(src).o -c $(src).c

elf: object
		avr-gcc $(cflags) -mmcu=$(avrType) -o $(src).elf $(src).o libtwi.a libuart.a
